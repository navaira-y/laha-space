...
async function loadTeachers() {
  ...
  try {
    ...
    grid.innerHTML = '<p style="color:#888">Unable to load teachers right now.</p>';
  }
}
...
function renderGrid(teachers) {
  const grid = document.getElementById('teacherGrid');
  if (!teachers.length) {
    grid.innerHTML = '<p style="color:#888;padding:40px 0">No teachers found.</p>';
    return;
  }
  grid.innerHTML = teachers.map(t => {
    ...
    return `
      <div class="teacher-card" data-id="${t.id}" data-categories='${JSON.stringify(cats)}'>
        ${photo}
...
async function openTeacherModal(id) {
  ...
  body.innerHTML = '<p style="padding:40px;color:#888">Loading...</p>';
  modal.hidden = false;
  ...
  body.innerHTML = `
    ...
    <div class="modal__tags">
      ${cats.map(c => `<span class="tag">${capitalize(c)}</span>`).join('')}
      ${langs.map(l => `<span class="tag" style="background:rgba(212,165,116,0.15);color:#9a6f3a">${l}</span>`).join('')}
    </div>
    ${t.laha_endorsement ? `<div class="modal__endorsement">"${t.laha_endorsement}"<br/><small style="font-style:normal;font-weight:600;color:#2F877D">— Laha Space</small></div>` : ''}
    ...
    <div class="modal__section">
      ...
      <div id="reviewForm">${buildReviewForm(t.id)}</div>
    </div>
  `;
}
  ...
  body.innerHTML = `
    ...
    <div id="bookingFormSection" style="display:none">
      ...
      <div class="form__group">
        ...
        <input class="form__input" id="bName" type="text" required />
      </div>
      <div class="form__group">
        <label class="form__label">Email *</label>
        <input class="form__input" id="bEmail" type="email" required />
      </div>
      <div class="form__group">
        <label class="form__label">Phone / WhatsApp *</label>
        <input class="form__input" id="bPhone" type="text" required />
      </div>
      <div class="form__group">
        <label class="form__label">What are you looking for? <span style="color:#888;font-size:13px">(optional)</span></label>
        <input class="form__input" id="bLooking" type="text" placeholder="e.g. Beginner, Tajweed revision..." />
      </div>
...
async function submitBooking() {
  ...
  if (!selectedSlot) {
    errEl.textContent = 'Please pick a time slot.';
    errEl.style.display = 'block';
    return;
  }
  ...
  if (data.success) {
    document.getElementById('bookingModalBody').innerHTML = `
      <div class="booking-success">
        ...
        <div class="booking-success__text">Your discovery session with ${selectedTeacher.name} on ${formatDateFull(selectedDate)} at ${formatTime(selectedSlot.start)} has been confirmed. The teacher will be in touch with you shortly. JazakAllahu khairan.</div>
      </div>
    `;
  } else {
  ...
  return `
    ...
    <div class="form__group">
      ...
      <div class="star-picker" id="starPicker" data-rating="0">
        ${[1,2,3,4,5].map(i => `<span class="star-pick" data-val="${i}" style="font-size:28px;cursor:pointer;color:#DFDFDF">★</span>`).join('')}
      </div>
...
async function submitReview(teacherId) {
  ...
  const res = await fetch('/api/reviews', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ teacher_id: teacherId, student_name: name, rating, review_text: text })
  });
  const data = await res.json();
  if (data.success) {
